...
import com.nasoft.brewer.model.StatusVenda;
import com.nasoft.brewer.model.TipoPessoa;
import com.nasoft.brewer.model.Venda;
import com.nasoft.brewer.repository.filter.VendaFilter;
import com.nasoft.brewer.repository.paginacao.PaginacaoUtil;
...
public class VendasImpl implements VendasQueries {
	...
	public Page<Venda> filtrar(VendaFilter filtro, Pageable pageable) {
		...
		if (pageable.getSort() == null )
			adicionarOrderByDefault(criteria, pageable, "dataCriacao");
		
		return new PageImpl<>(criteria.list(), pageable, total(filtro));
	}
	
	private void adicionarOrderByDefault(Criteria criteria, Pageable pageable, String orderDefault) {
		Sort sort = pageable.getSort();
		if (sort != null) {
			Sort.Order order = sort.iterator().next();
			String property = order.getProperty();
			criteria.addOrder(order.isAscending() ? Order.asc(property) : Order.desc(property));
		} else if (orderDefault != null){
			criteria.addOrder(Order.desc(orderDefault));
		}
	...
	public Long qtdeItensEstoque() {
		Optional<Long> optional = Optional.ofNullable(
				manager.createQuery("select sum(quantidadeEstoque) from Cerveja", Long.class)
					.getSingleResult());
	...
	public Long qtdeTotalClientes() {
		Optional<Long> optional = Optional.ofNullable(
				manager.createQuery("select count(1) from Cliente", Long.class)
					.getSingleResult());
		return optional.orElse(0L);
	}
	...
	public BigDecimal valorEstoque() {
		Optional<BigDecimal> optional = Optional.ofNullable(
				manager.createQuery("select sum(valor*quantidadeEstoque) from Cerveja", BigDecimal.class)
					.getSingleResult());
		return optional.orElse(BigDecimal.ZERO);
	}
	...
	public List<VendaMes> totalPorMes() {
		...
		/*
		deve configurar no arquivo JPAConfig o bean o EntityManagerFactory, contendo a especificacao do arquivo que contenha o sql externo 
		...
		*/
		...
		//esse for serve para incluir meses que porventura não retornem na consulta, a fim de evitar buracos no grafico
